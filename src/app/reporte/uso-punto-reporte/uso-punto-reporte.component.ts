...
import { NOTIFY } from 'src/app/commons/app-utils';
...
@Component({
  selector: 'app-uso-punto-reporte',
  templateUrl: './uso-punto-reporte.component.html',
  styleUrls: ['./uso-punto-reporte.component.css']
})
export class UsoPuntoReporteComponent implements OnInit {
  ...
  fileName = 'uso-punto-reporte.xlsx';
  ...
  iniciarTabla() {
    this.tableData1 = {
      headerRow: ['Id', 'Concepto de Uso', 'Cant. Requerida', 'Nombre', 'Apellido', 'Puntaje Utilizado'],
      dataRows: this.listaUsoPunto
    };
  ...
  getUsoPuntos(clienteId) {
    ...
    this.reporteService.getUsoPuntoReporte(body).subscribe(
      response => {
        this.listaUsoPunto = new Array<any>();
        this.lista = new Array<any>();
        if (response.data.cabeceras.length > 0) {
          response.data.cabeceras.forEach(cabecera => {
            this.listaAtributos = new Array<any>();
            cabecera.usoDetalle.forEach(usoDetalle => {
              this.listaAtributos.push(cabecera.id);
              this.listaAtributos.push(cabecera.vale.descripcion);
              this.listaAtributos.push(cabecera.vale.cantidadRequerida);
              this.listaAtributos.push(usoDetalle.bolsaPuntos.cliente.nombre);
              this.listaAtributos.push(usoDetalle.bolsaPuntos.cliente.apellido);
              this.listaAtributos.push(usoDetalle.puntajeUtilizado);
            });
            this.listaUsoPunto.push(this.listaAtributos);
            this.tableData1 = {
              headerRow: ['Id', 'Concepto de Uso', 'Cant. Requerida', 'Nombre', 'Apellido', 'Puntaje Utilizado'],
              dataRows: this.listaUsoPunto
            };
  ...
  exportarPdf() {
    ...
    doc.save('uso-punto-reporte.pdf');
  }
  ...
  aceptarCliente() {
    ...
    this.getUsoPuntos(this.clienteId);
